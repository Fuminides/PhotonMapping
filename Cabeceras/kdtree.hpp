...
//No se como poner esto de aqui exactamente para especificar que es de fotones. 
template<Foton> struct kdtree;

/* Crea un kdtree sin ningún elemento, vacío*/
void crear(kdtree<Foton>& arbol);

/* Ha insertado el elemento 'e' en el kdtree 'arbol'. Si había un
   elemento igual a 'e' previamente, lo ha sobreescrito*/
void anyadir(kdtree<Foton>& arbol,Foton e);

/* Devuelve verdad si y solo si en el kdtree 'arbol' hay un
   elemento igual al elemento 'e'. En caso contrario devuelve falso*/
bool esta(const kdtree<Foton>& arbol,Foton e);

/* Si en 'arbol' hay un elemento igual a 'e' devuelve ese elemento, en caso 
   contrario el parámetro error que se pasa por referencia se pondrá a true*/
Foton obtener(const kdtree<Foton>& arbol, Foton e, 
                                     bool &error);

/* Devuelve el número de elementos que hay en el kdtree 'arbol'*/
int tamanyo(const kdtree<Foton>& arbol);

/* Devuelve true y solo si el kdtree 'arbol' no tiene ningún 
   elemento, en caso contrario devuelve falso*/
bool esVacia(const kdtree<Foton>& arbol);

/*
 * Devuelve los k fotones mas proximos dado un punto.
 */
std::vector<Foton> knearest(const kdtree<Foton>& arbol, Punto p);

//------------------ITERADOR---------------------------
/* Inicializa el iterador para poder recorrer los elementos de 'a', de 
   forma que el siguiente elemento a visitar sea el primero */
void iniciarIterador(kdtree<Foton> &a);
/* Devuelve true si y solo si queda algun elemento por visitar del kdtree 'a'*/ 
 bool existeSiguiente(kdtree<Foton> &a);

/* En caso de que queden elementos por visitar de 'a', devuelve el siguiente 
   elemento por vistar y avanza el iterador, en caso contrario, el parámetro 
   pasado por referencia 'error' pasará a valer true*/
...
void insertarRec(typename kdtree<Foton>::NodoBinario*& nodo, Foton e, int nivel);
...
/* Ha buscado de forma recursiva el elemento 'e' a partir del nodo 'nodo' que se
 ...
 bool estarRec(typename kdtree<Foton>::NodoBinario* nodo, Foton e);
...
struct kdtree {
    friend void crear(kdtree<Foton>& arbol);
    friend void anyadir(kdtree<Foton>&arbol,Foton e);
    friend bool esta(const kdtree<Foton>& arbol,Foton e);
    friend Foton obtener(const kdtree<Foton>& arbol, Foton e,bool &error);
    friend bool esVacia(const kdtree<Foton>& arbol);
    friend int tamanyo(const kdtree<Foton>& arbol);
 ...
 private:
  ...
  /*DECLARACION DE OPERACIONES INTERNAS */
  friend void insertarRec(typename kdtree<Foton>::NodoBinario*& nodo, Foton e, int nivel);
  friend bool estarRec(typename kdtree<Foton>::NodoBinario* nodo, Foton e);
  friend Foton obtenerRec(typename kdtree<Foton>::NodoBinario* nodo, Foton e, bool &error);
...
void insertarRec(typename kdtree<Foton>::NodoBinario*& nodo, Foton e, int nivel) {
    if (nodo==nullptr)   {
        nodo=new typename kdtree<Foton>::NodoBinario;
        nodo->dato=e;
        nodo->izq=nullptr;
        nodo->dcho=nullptr;
        Vector normal;
        if ( nivel % 2 == 0 ) normal.set_values(0,0,1);
        else normal.set_values(0,1,0);
        nodo->plano.set_values(e.getPosicion(), normal, NULL, 0, NULL, 0);
    }
    else {
...
Foton obtener(const kdtree<Foton>& arbol, Foton e, bool &error) {
